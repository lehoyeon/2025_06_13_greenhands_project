...
from config import REPORT_DIR # config.py에서 REPORT_DIR 임포트

logger = logging.getLogger(__name__)

def generate_excel_report(subject: str = "농작물", content_for_report: str = None): 
    """
...
def generate_word_report(subject: str = "농작물", content_for_report: str = None):
    """
    워드 보고서를 생성하고, 저장된 파일의 URL (정적 서빙 경로)을 반환합니다.
    """
    try:
        document = Document()
        ...
        logger.error("Error: 'python-docx' library not installed. Please run 'pip install python-docx'.")
        raise HTTPException(status_code=500, detail="docx 라이브러리가 설치되지 않았습니다. 'pip install python-docx'를 실행하여 설치해주세요.")
    except Exception as e:
        logger.error(f"Error generating word report: {type(e).__name__}: {e}")

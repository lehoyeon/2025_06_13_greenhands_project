...
async function loadUserInfo() {
    try {
        ...
        if (response.ok) {
            ...
            // harvested_crops.html에도 이 span이 있을 수 있으므로 함께 처리
            const usernameDisplayHarvested = document.getElementById('current-username-display-harvested');

            if (usernameDisplay) {
                const displayUserName = userData.nickname && userData.nickname !== 'null' && userData.nickname !== '' ? userData.nickname : (userData.username || '사용자');
                usernameDisplay.textContent = `${displayUserName}님의 농장`;
            }
            if (usernameDisplayHarvested) { // 새로운 페이지의 요소
                const displayUserName = userData.nickname && userData.nickname !== 'null' && userData.nickname !== '' ? userData.nickname : (userData.username || '사용자');
                usernameDisplayHarvested.textContent = `${displayUserName}님의 수확 작물`;
            }
            ...
            const usernameDisplayHarvested = document.getElementById('current-username-display-harvested');
            if(usernameDisplayHarvested) usernameDisplayHarvested.textContent = '정보 로드 실패';
        }
    } catch (error) {

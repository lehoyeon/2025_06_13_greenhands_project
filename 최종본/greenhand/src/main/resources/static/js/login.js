     ...
     resetPwForm.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                if (response.ok && contentType.includes("application/json")) {
                    const data = await response.json();
                    // ⭐⭐⭐ 이 부분 수정 시작 ⭐⭐⭐
                    // 백엔드에서 직접 비밀번호를 보내준다고 가정
                    if (data.newPassword) { // 백엔드에서 'newPassword'라는 키로 새 비밀번호를 보낼 경우
                        displayResult(resetPwResult, `새로운 비밀번호: ${data.newPassword} (로그인 후 변경 권장)`, 'success');
                    } else if (data.message) { // 기존처럼 'message' 키만 있다면 그대로 사용
                        displayResult(resetPwResult, data.message, 'success');
                    } else {
                        displayResult(resetPwResult, "비밀번호 재설정 성공. 그러나 새 비밀번호 정보가 없습니다.", 'success');
                    }
                    // ⭐⭐⭐ 이 부분 수정 끝 ⭐⭐⭐
                } else {
                    const errorText = await response.text();

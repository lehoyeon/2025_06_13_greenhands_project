...
function addMessageAndSave(senderType, content, isUser, imageUrlFromServer = null, fileUrlFromServer = null, linkUrl = null) {
    const messageObject = {
        ...
        linkUrl: linkUrl // ⭐ linkUrl 저장
    };

    chatHistoryArray.push(messageObject);
    sessionStorage.setItem('chatHistory', JSON.stringify(chatHistoryArray));

    addMessageToChat(senderType, content, isUser, null, imageUrlFromServer, fileUrlFromServer, linkUrl); // ⭐ linkUrl 전달
...
function addMessageToChat(sender, messageHtmlContent, isUser = false, imageFile = null, imageUrlFromServer = null, fileUrlFromServer = null, linkUrl = null) {
    ...
    if (imageFile) { // 클라이언트에서 선택한 이미지 미리보기 (사용자 메시지용)
        ...
        // ⭐⭐⭐ 링크 생성 로직 수정 및 강화 ⭐⭐⭐
        if (linkUrl) {
            // knowledge_base에서 정의한 link_keyword를 찾아 해당 부분에 링크를 삽입합니다.
            // link_keyword는 백엔드에서 넘겨주는 botResponseText 안에 이미 포함되어 있을 것으로 가정합니다.
            // 예: "질병 진단 바로가기" -> <a href="...">질병 진단 바로가기</a>
            // 백엔드에서 전달되는 link_url에 따라 base URL을 동적으로 설정합니다.
            let baseUrl = "http://localhost:8080/"; // 기본 Spring Boot 웹 페이지 경로
            const fullLinkUrl = `${baseUrl}${linkUrl}`;

            // link_keyword는 백엔드에서 보낸 messageHtmlContent 안에 포함되어 있으므로
            // messageHtmlContent에서 "바로가기" 텍스트를 찾아 링크로 만듭니다.
            // (agricultural_knowledge_base의 details 내용을 따름)
            const keywordRegex = /\[(.*?) 바로가기\]/g; // "[XXX 바로가기]" 패턴을 찾습니다.
            const match = keywordRegex.exec(finalContent);
            if (match && match[1]) {
                const linkText = `${match[1]} 바로가기`; // 매치된 그룹 텍스트에 " 바로가기"를 붙여 실제 링크 텍스트를 만듭니다.
                finalContent = finalContent.replace(
                    keywordRegex,
                    `<a href="${fullLinkUrl}" target="_blank" class="chat-link-button">${linkText}</a>`
                );
                console.log(`[Chatbot UI] 텍스트 메시지 내 링크 키워드 감지 및 링크 추가 완료. URL: ${fullLinkUrl}, 텍스트: ${linkText}`);
            } else {
                 // 만약 특정 키워드를 찾지 못했으나 linkUrl이 있다면, 메시지 끝에 "해당 페이지로 이동" 같은 링크를 추가할 수 있습니다.
                 // 이 부분은 필요에 따라 구현하세요.
                 // 예: finalContent += `<br><br><a href="${fullLinkUrl}" target="_blank" class="chat-link-button">해당 페이지로 이동</a>`;
                 console.warn(`[Chatbot UI] 메시지 내 링크 키워드를 찾을 수 없어 링크를 추가하지 않음. URL: ${fullLinkUrl}`);
            }
        }
...
async function sendMessage() {
    ...
    try {
        ...
        let botResponseLinkUrl = data.link_url || null; // ⭐ link_url 추출

        // 봇 응답을 chatHistoryArray에 저장하고 세션 스토리지에 반영
        addMessageAndSave('bot', botResponseText, false, botResponseImageUrl, botResponseFilePath, botResponseLinkUrl); // ⭐ link_url 전달
    ...
    try {
        ...
        if (history.length === 0) {
            ...
            history.forEach(item => {
                ...
                if (item.link_url) { // ⭐ link_url 처리: history_list에서도 페이지 링크가 보이도록
                    const baseUrl = "http://localhost:8080/";
                    const histFullLinkUrl = `${baseUrl}${item.link_url}`;
                    // history_list에 표시될 텍스트는 bot_response에서 추출하거나, 명시적으로 설정
                    // 여기서는 간단히 "페이지 바로가기"로 표시하거나, bot_response에서 link_keyword를 찾아 적용할 수 있습니다.
                    const linkMatch = item.bot_response.match(/\[(.*?) 바로가기\]/);
                    const linkText = linkMatch ? `${linkMatch[1]} 바로가기` : "해당 페이지로 이동";

                    historyContent += `<br><a href="${histFullLinkUrl}" target="_blank" class="chat-link-button history-link">${linkText}</a>`;
                }

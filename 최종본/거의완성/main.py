...
# ⭐ 변경: 로깅 레벨을 INFO로 설정 (DEBUG 로그 줄이기 위함)
...
# FastAPI가 직접 서빙하는 정적 파일은 uploaded_images, reports 뿐입니다.
...
# ⭐⭐⭐ 추가된 부분 ⭐⭐⭐
# 'images' 폴더의 이미지를 '/images' URL 경로로 서빙하도록 설정
# 여기서 'directory="images"'는 FastAPI 프로젝트의 루트 폴더 바로 아래에 있는 'images' 폴더를 의미합니다.
# 이 폴더에 knowledge_base.py에서 참조하는 썸네일 이미지(예: /img/lettuce.jpg)가 있어야 합니다.
# (참고: /img/는 기존 knowledge_base.py에서 사용한 경로이므로, 실제 폴더는 /images/가 맞을 것으로 가정합니다.)
app.mount("/img", StaticFiles(directory="images"), name="images") # ⭐ /img 경로로 변경 ⭐
# ⭐⭐⭐ 여기까지 추가 ⭐⭐⭐
...
async def download_file(path_in_uploads_or_reports: str):
    ...
    if path_in_uploads_or_reports.startswith("uploaded_images/"):
        ...
        # 허용되지 않은 경로 접근 시 403 Forbidden 반환
...
if __name__ == "__main__":
    # 필요한 디렉토리 생성 (혹시 main.py 직접 실행 시에도 보장)
    os.makedirs(UPLOAD_IMAGE_DIR, exist_ok=True)
    os.makedirs(REPORT_DIR, exist_ok=True)
    ...
    db = SessionLocal()
    try:
        # get_or_create_user 함수는 user_id, username을 받으므로, 이 부분을 맞춰줍니다.
        # nickname이나 name은 database.py의 get_or_create_user 내부에서 기본값이 설정됩니다.

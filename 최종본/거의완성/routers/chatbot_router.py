...
async def ask_chatbot_endpoint(
    ...
    bot_file_path = None
    bot_link_url = None # ⭐ 추가: 봇 링크 URL 변수 초기화
    ...
    if is_explicit_excel_request:
        ...
        found_in_knowledge_base_or_ai_response = False # ⭐ 변수명 변경 및 초기화

        # ⭐⭐ knowledge_base에서 직접 링크 정보를 확인하는 로직 (ai_service.py에서 이미 처리하지만, 안전장치로 한번 더 확인 가능) ⭐⭐
        # ai_service.py에서 이 로직을 선행 처리하므로, 여기서는 단순히 ai_service.py의 결과를 기다립니다.

        # ai_service.py의 get_gemini_response_for_chat는 이제 딕셔너리를 반환합니다.
        ai_response_data = await get_gemini_response_for_chat(user_query, image_url_for_db) # ⭐ 함수 호출 수정
        bot_response_content = ai_response_data.get("response", "응답 내용을 찾을 수 없습니다.")
        bot_link_url = ai_response_data.get("link_url", None) # ⭐ link_url 추출
    ...
    try:
        ...
        if bot_response_content.strip():
            bot_log = ChatLog(user_id=user_id, message_type="bot",
                              ...
                              link_url=bot_link_url, # ⭐ link_url 저장
    ...
    return {"response": bot_response_content, "file_path": bot_file_path, "image_url": image_url_for_db, "link_url": bot_link_url} # ⭐ link_url 추가하여 반환
    ...
    try:
        ...
        for log in results:
            if log.message_type == 'user':
                ...
                user_message_buffer["link_url"] = log.link_url # ⭐ link_url 로드
